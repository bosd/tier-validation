...
- Resolving the lowest still-open review sequence no longer browses
  each underlying record one at a time. Both the promotion check and
  the ``can_review`` compute now group over ``tier.review`` in a single
...
- The systray review counter no longer recounts once per bus
  ...
  reviewer, and each recount costs a few queries per pending
  review, so a bulk approval could occupy every HTTP worker.
  Bus notifications are now coalesced (15s, leading + trailing)
  and concurrent recounts can no longer stack.
